 ...
 *  - 본문 최상단에 "본문 바로가기" 스킵 링크를 두어, 키보드 사용자가 헤더의
 *    로고·CTA 를 Tab 으로 통과하지 않고 곧바로 본문(#main-content)으로 갈 수 있게 한다.
...
import ScrollResetOnLoad from "./_components/ScrollResetOnLoad";
 ...
 * themeColor 는 모바일 브라우저 주소창 색이다.
 *
 * ★구글폼 13번(블루 계열) 반영: 기존 브랜드 그린 #03c75a 를 primary 블루로 교체했다.
 *   여기만 CSS 토큰을 쓸 수 없다. metadata/viewport 는 서버에서 정적 문자열로
 *   직렬화되어 <meta> 태그가 되므로 var(--color-primary) 를 해석할 주체가 없다.
 *   따라서 globals.css 의 --color-primary 값과 **수동으로 동일하게 유지**해야 한다.
 *   주조색을 바꿀 때는 globals.css 와 이 줄을 함께 고칠 것.
 * -------------------------------------------------------------------------- */
    ...
    >
      ...
      */}
      <body className="flex min-h-full flex-col bg-white text-ink antialiased">
        {/*
          새로고침 시 브라우저의 스크롤 위치 복원을 끄고 맨 위에서 시작하게 한다.
          UI 를 그리지 않는 부수효과 전용 클라이언트 컴포넌트다(자세한 배경은 파일 주석 참고).
          해시(#consult 등)가 붙은 주소로 들어온 경우는 예외로 두어 앵커 이동을 방해하지 않는다.
        */}
        <ScrollResetOnLoad />
        ...
        {/* 전 페이지 공통 고정 헤더 (position: fixed, 높이 80px)
            랜딩페이지라 섹션 이동 메뉴는 두지 않고 로고 + '상담 신청' 버튼만 있다. */}
        <Header />
