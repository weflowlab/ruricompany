      ...
      */}
      <main id="main-content" className="flex-1">
        ...
        <VehicleSection
          ...
          /* 모바일에서 4대를 가로 한 줄 스크롤로 (2026-07-31 요청) */
          mobileRow
        />

        {/* ── 4. 텍스트 배너 ───────────────────────────────────────────────────
            차량 리스트 사이를 끊어 주는 가로 띠.
            ★ 2026-07-31 primary 단색 → 다크 + 스포트라이트(.bg-spotlight)로 교체.
              히어로에만 다크를 쓰면 그 화면만 튄 것처럼 보인다. 페이지 중간에
              한 번 더 같은 처리를 넣어야 다크가 의도된 리듬으로 읽힌다.
            스크롤 진입 시 배지 → 메인 카피 순서로 페이드업한다. */}
        <TextBanner />

        {/* ── 5. 일반 화물차 (id="cargo") ──────────────────────────────────────
            두 번째 차량 리스트 섹션. 3번 섹션과 동일한 그리드를 재사용한다.
            ★ surface prop 제거 (2026-07-31): 바로 위 텍스트 배너가 다크로 바뀌면서
              이 섹션은 흰 배경일 때 대비가 가장 크다. 파스텔(surface)을 깔면
              다크 → 파스텔로 명도가 어중간하게 이어져 경계가 흐려진다. */}
        <VehicleSection
          id="cargo"
          title="용도별로 고르는 "
          highlight="일반 화물차"
