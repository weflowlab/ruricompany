 ...
 *    (구글폼 3번 업종 변경으로 질문 내용이 신차 할부·리스·장기렌트 공통 질문으로 교체됐다.
 *     아코디언 개폐 로직과 접근성 처리는 그대로 유지하고 문구만 다듬었다.)
  ...
  return (
    ...
    >
      <div className="mx-auto w-full max-w-[1320px] px-5">
        ...
        {/*
          섹션 제목 — 구글폼 3번(신차 할부·리스·장기렌트) 업종에 맞춰
          "세 가지 방식"을 명시해 어떤 방식을 알아보는 분이든 답이 있음을 알린다.
          8-1번(금액 노출 금지)에 따라 "견적", "비용" 같은 단어는 쓰지 않는다.
        */}
        <div id="faq-heading">
          <SectionTitle
            title="상담 전에 많이 묻는 "
            highlight="질문"
            description="할부·리스·장기렌트를 알아보실 때 자주 나오는 질문을 모았습니다."
          />
        </div>
        ...
        */}
        ...
        >
          ...
          {/*
            하단 CTA 한 줄 — 아코디언에서 답을 못 찾은 사용자를 상담으로 넘기는 마지막 통로.
            구글폼 4번(상담 문의 유도)에 맞춰 질문형 한 줄로 문턱을 낮추고,
            경로는 하단 상담 폼(#consult) 하나로만 남겼다. 전화번호는 헤더·플로팅 CTA·
            푸터에 이미 있어 여기서 또 노출하면 선택지만 늘어난다(8-1번 "조잡함" 회피).
            버튼을 새로 추가하지 않고 문장 안의 링크로만 처리해 요소 수를 늘리지 않았다.
          */}
