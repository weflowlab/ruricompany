 ...
 *
 * 레이아웃
 *  - 차량이 9대이므로 태블릿 이상에서 3열 × 3행 정사각 그리드로 딱 떨어진다.
 *    grid-cols-2 gap-5 → md:grid-cols-3 md:gap-8
 *    이전에는 8대를 4열 × 2행으로 깔았는데, 열을 3으로 줄이면 카드 한 장의
 *    가로폭이 넓어져 차량명·트림·태그가 덜 눌린다.
 *    모바일은 2열이라 마지막 행에 카드 한 장만 남지만(9 = 2×4 + 1),
 *    카드 폭이 행마다 같아 어색하지 않다.
 *
 * 인터랙션
    ...
    >
      ...
      <div className="mx-auto w-full max-w-[1320px] px-5">
        ...
        */}
        <ul
          ref={gridRef}
          className="grid grid-cols-2 items-stretch gap-5 md:grid-cols-3 md:gap-8"
        >
